...
class TfPoseEstimator:
    ...
    def draw_humans(npimg, humans, imgcopy=False):
        ...
        for human in humans:
            ...
            for i in range(common.CocoPart.Background.value):
                ...
                cv2.putText(npimg, str(i), (center[0] + 20, center[1] + 20),
    ...
    def get_humans(npimg, humans, imgcopy=False):
        ...
        bboxes = []
        xcenter = []
        sk = np.zeros_like(npimg, dtype='uint8')
        for human in humans:
            xs = []
            ys = []
            centers = {}
            ...
            for i in range(common.CocoPart.Background.value):
                ...
                ys.append(center[1])
                cv2.circle(npimg, center, 3, common.CocoColors[i], thickness=3, lineType=8, shift=0)
                # cv2.putText(npimg, str(i), (center[0]+20, center[1]+20), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 255), 1)
            ...
            for pair_order, pair in enumerate(common.CocoPairsRender):
                if pair[0] not in human.body_parts.keys() or pair[1] not in human.body_parts.keys():
                    continue
                cv2.line(npimg, centers[pair[0]], centers[pair[1]], common.CocoColors[pair_order], 3)
                cv2.line(sk, centers[pair[0]], centers[pair[1]], (255, 0, 0), 3)
